...
| Tooling | **Improving** | Helper scripts exist and the regenerated registries keep `npm test` green.
...
2. **Automate Registry Drift Checks** – Add lint commands or CI tasks that verify serialized datasets match source data.
...
- Introduce a lightweight automation step (script or CI job) that asserts registry files remain in sync with their upstream data sources.
