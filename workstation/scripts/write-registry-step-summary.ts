...
export function readSummaryContent(reportPath: string, options: SummaryReadOptions = {}): string {
  ...
  try {
    ...
    options.onWarning?.(`Registry report read failed for ${reportPath}: ${message}`)
    return unreadableReportSummary(reportPath)
  }
}

export function writeStepSummary(summaryPath: string, reportPath: string, options: SummaryReadOptions = {}): void {
  fs.appendFileSync(summaryPath, readSummaryContent(reportPath, options))
}
...
export function main(): void {
  ...
  writeStepSummary(summaryPath, reportPath, { onWarning: console.warn })
}
