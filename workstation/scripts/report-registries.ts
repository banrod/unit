...
function main(): void {
  ...
  const specBackedUnitRefs: string[] = []
  ...
  for (const [specId, spec] of specEntries) {
    ...
    for (const unit of unitValues) {
      ...
      if (specIds.has(unitId)) {
        specBackedUnitRefs.push(unitId)
      } else {
  ...
  if (missingUnitClasses.size > 0) {
    const missingUnitClassFrequency = sortByFrequency(missingUnitClassRefs).map(
    ...
    issues.push({
      ...
      message: `Spec units reference IDs missing from both class and spec registries: ${formatList(missingUnitClassFrequency)}`,
    })
  }

  if (specBackedUnitRefs.length > 0) {
    const specBackedFrequency = sortByFrequency(specBackedUnitRefs).map(
      ([id, count]) => `${describeId(id, idNameIndex)} ×${count}`
    )

    issues.push({
      level: 'warn',
      count: new Set(specBackedUnitRefs).size,
  ...
  if (specsMissingFromIds.size > 0) {
    issues.push({
      ...
      message: `Spec IDs not exported from _ids.ts: ${formatList(describeIds(Array.from(specsMissingFromIds), idNameIndex))}`,
    })
  }
