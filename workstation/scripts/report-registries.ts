...
import { describeId, runRegistryAudit } from './registry-audit'

const PREVIEW_LIMIT = 25

function previewList<T>(values: T[], limit: number = PREVIEW_LIMIT): { total: number; preview: T[] } {
  return {
    total: values.length,
    preview: values.slice(0, limit),
  }
}
...
function writeJsonReport(result: ReturnType<typeof runRegistryAudit>): void {
  ...
  const jsonReport = {
    ...
    gaps: {
      idsMissingClass: previewList(result.idsMissingClass),
      idsMissingComponent: previewList(result.idsMissingComponent),
      specsMissingFromIds: previewList(result.specsMissingFromIds),
      unresolvedUnitRefs: previewList(
        result.unresolvedUnitRefFrequency.map(([id, count]) => ({
          id,
          names: result.idNameIndex[id] ?? [],
          count,
        }))
      ),
      compositeSpecRefs: previewList(
        result.compositeSpecRefFrequency.map(([id, count]) => ({
          id,
          names: result.idNameIndex[id] ?? [],
          count,
        }))
...
function main(): void {
  ...
  lines.push('', '## Top Composite Spec References')

  if (result.compositeSpecRefFrequency.length === 0) {
    lines.push('- None detected.')
  } else {
    for (const [id, count] of result.compositeSpecRefFrequency.slice(0, 10)) {
      lines.push(`- ${describeId(id, result.idNameIndex)} ×${count}`)
    }
  }
