...
function writeJsonReport(result: ReturnType<typeof runRegistryAudit>): void {
  ...
  const jsonReport = {
    ...
    gaps: {
      idsMissingClass: result.idsMissingClass,
      idsMissingComponent: result.idsMissingComponent,
      specsMissingFromIds: result.specsMissingFromIds,
      unresolvedUnitRefs: result.unresolvedUnitRefFrequency.map(([id, count]) => ({
        id,
        names: result.idNameIndex[id] ?? [],
        count,
      })),
      compositeSpecRefs: result.compositeSpecRefFrequency.map(([id, count]) => ({
        id,
        names: result.idNameIndex[id] ?? [],
        count,
      })),
