...
option checks, report generation, and registry-report artifact upload).
...
lives in `notes/registry-report.md` (and `notes/registry-report.json` for machine-readable automation with schema, issue summary, and structured gap details). To profile the image-filter prototype at
